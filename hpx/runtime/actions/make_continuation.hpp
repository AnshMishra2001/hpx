...
namespace hpx {
    ...
    inline typename std::enable_if<!std::is_same<typename std::decay<F>::type,
                                       hpx::naming::id_type>::value,
    ...
    make_continuation(Cont&& cont, hpx::naming::id_type const& target, F&& f)
    {
        typedef typename std::decay<Cont>::type cont_type;
        typedef typename std::decay<F>::type function_type;
